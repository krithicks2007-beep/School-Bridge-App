            ...
            <View key={rowIndex} className="mb-3 flex-row justify-between">
              {week.map((day, colIndex) => {
                ...
                return (
                  <TouchableOpacity 
                    key={colIndex} 
                    className={`h-[38px] w-[38px] items-center justify-center rounded-[14px] ${isSelected ? 'border-2 border-slate-800 shadow-sm' : ''}`}
                    style={day ? { backgroundColor: status === 'N/A' ? '#F1F5F9' : getStatusColor(status) + '15' } : undefined}
                    onPress={() => handleDateClick(day)}
                    disabled={!day}
                  >
                    <Text
                      className={`text-[15px] ${isSelected ? 'font-black' : 'font-semibold'}`}
                      style={day ? { color: status === 'N/A' ? '#94A3B8' : getStatusColor(status) } : undefined}
                    >
                      {day || ''}
                    </Text>
                  </TouchableOpacity>
