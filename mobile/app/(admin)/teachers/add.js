  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok && !result.error) {
        ...
        if (Platform.OS === 'web') {
          window.alert(result.error || 'Failed to add teacher.');
        } else {
          Alert.alert('Error', result.error || 'Failed to add teacher.');
        }
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <View className="flex-1 bg-white" style={{ paddingTop: insets.top + 20 }}>
      <View className="px-5 pb-4 flex-row items-center border-b border-gray-100">
        ...
        <Text className="text-xl font-bold text-brand-950">Add Teacher</Text>
      </View>
      ...
      >
        <ScrollView className="flex-1 px-5 pt-4" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <TouchableOpacity
            className={`mb-10 items-center justify-center rounded-xl p-4 shadow-sm ${saving ? 'bg-purple-400' : 'bg-purple-600'}`}
          ...
          >
            {saving ? (
              ...
              <Text className="text-base font-bold text-white">Save Teacher</Text>
            )}
          </TouchableOpacity>
