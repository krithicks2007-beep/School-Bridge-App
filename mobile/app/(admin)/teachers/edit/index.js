...
export default function EditTeacherSearch() {
  ...
  const renderTeacherItem = ({ item }) => (
    <View className="mb-3 flex-row items-center justify-between rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
      ...
      <TouchableOpacity
        className="flex-row items-center justify-center rounded-lg bg-indigo-50 px-4 py-2"
        onPress={() => router.push(`/(admin)/teachers/edit/${item.reg_id}`)}
      >
        <Feather name="edit-2" size={16} color="#4F46E5" />
        <Text className="ml-2 font-semibold text-indigo-600">Edit</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    <View className="flex-1 bg-white" style={{ paddingTop: insets.top + 20 }}>
      <View className="px-5 pb-4 flex-row items-center border-b border-gray-100">
        ...
        <Text className="text-xl font-bold text-brand-950">Select Teacher to Edit</Text>
      </View>
